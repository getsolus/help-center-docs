...
```bash
gpg --verify Solus-Budgie-Release-2026-04-18.iso.sha256sum.sign Solus-Budgie-Release-2026-04-18.iso.sha256sum
```
...
```bash
sha256sum -c Solus-Budgie-Release-2026-04-18.iso.sha256sum | grep OK
```
...
```powershell
Get-FileHash C:\path\to\Solus-Budgie-Release-2026-04-18.iso
cat C:\path\to\Solus-Budgie-Release-2026-04-18.iso.sha256sum
```
...
```bash
sudo dd if=Solus-Budgie-Release-2026-04-18.iso of=/dev/sdb bs=4M status=progress oflag=sync && sudo eject /dev/sdb
```
...
3. Click on “Burn Disk Image ‘Solus-Budgie-Release-2026-04-18.iso’ to Disc…”
4. Then click “Burn”.
...
```bash
sudo dd if=Solus-Budgie-Release-2026-04-18.iso of=/dev/rIDENTIFIER bs=1m
```
